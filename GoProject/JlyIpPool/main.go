...
var log = util.NewInitLog("/data/kdl/log/devops/JlyIpPool.log")
...
func init() {
	var err error
	Db, err = util.JlynodeDB()
	if err != nil {
		panic(err)
	}
	rdb, err = util.JlyRedisDB()
	if err != nil {
		panic(err)
	}
	newObj()
...
func getNode() (res []NodeData, e error) {
	sqlStr := "select public_ip,UNIX_TIMESTAMP(next_change_time) AS next_change_time,source from node where status=1 and role>=8 AND role<=15 and public_ip<>'' and exclude_for_transfer=0 and enable=1"
