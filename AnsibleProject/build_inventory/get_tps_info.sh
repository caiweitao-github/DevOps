#!/bin/bash
file_name="/home/weitaocai/ansible/tps_inventory"
> $file_name
st="tps"
echo "tps:" > $file_name
...
get_code_info(){
    ip=$($mysql -N -e 'select login_ip from tps where code = "'$i'";')
    port=$($mysql -N -e 'select login_port from tps where code = "'$i'";')
